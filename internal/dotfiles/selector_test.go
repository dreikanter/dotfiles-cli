...
func sampleSpecs(home string) []Spec {
	return []Spec{
		{Tool: "git", LocalRoot: filepath.Join(home, ".gitconfig"), DotfileRoot: "/repo/config/git/.gitconfig"},
		{Tool: "git", LocalRoot: filepath.Join(home, ".gitignore_global"), DotfileRoot: "/repo/config/git/.gitignore_global"},
		{Tool: "shell", LocalRoot: filepath.Join(home, ".zshrc"), DotfileRoot: "/repo/config/shell/.zshrc"},
		{Tool: "nvim", LocalRoot: filepath.Join(home, ".config/nvim"), DotfileRoot: "/repo/config/nvim", IsDir: true},
	}
}
...
func TestSelector_Apply_ToolAndFile(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, got, 1)
	assert.Equal(t, filepath.Join(home, ".gitconfig"), got[0].LocalRoot)
}
...
func TestSelector_Apply_FileTrailingSlashIgnored(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, got, 1)
	assert.Equal(t, filepath.Join(home, ".config/nvim"), got[0].LocalRoot)
}
...
func TestSelector_Apply_FileCWDRelative(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, got, 1)
	assert.Equal(t, filepath.Join(dir, "f"), got[0].LocalRoot)
}
